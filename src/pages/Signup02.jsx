...
const Signup = () => {
  ...
  const executeRecaptcha = () => {
    return new Promise((resolve, reject) => {
      window.grecaptcha.ready(() => {
        window.grecaptcha.execute('6LfkvD4rAAAAAPJPSvnKaHCvLej0hRotvj3TOYmA', { action: 'signup' })
  ...
  const sendVerificationSMS = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8000/account/phone/phone_verification/', {
        method: 'POST',
        headers: {
  ...
  try {
    ...
    const verifyResponse = await fetch('http://localhost:8000/account/phone/verify_code/', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
