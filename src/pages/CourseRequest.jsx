  ...
  return (
    ...
    >
      ...
      <p className="text-blue-200 mb-2">
        <span className="font-medium text-blue-300">{t("Language")}:</span>{" "}
        {t(course.language)}
      </p>
      <p className="text-blue-200 mb-2">
        <span className="font-medium text-blue-300">{t("Level")}:</span>{" "}
        {t(course.level)}
      </p>
      <button
...
const RequestPage = () => {
  ...
  const searchForExistingCourses = async (prompt) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://api.tadrisino.org/courses/course-generation/search_for_existence_course/",
        {
          method: "POST",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://api.tadrisino.org/courses/course-generation/generate_gpt_course/",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        ...
        if (
          response.status === 403 &&
          errorData.detail === "you do not have enough credit"
        ) {
        ...
        >
          ...
          <div className="max-h-60 overflow-y-auto">
            {searchResults.map((course) => (
              <div
                key={course.id}
                className="border-b border-white border-opacity-20 py-2 last:border-0"
              >
          ...
          <p className="text-blue-200 mt-3 text-sm">
            {t(
              "Not what you were looking for? Create a new course by submitting your request."
            )}
          </p>
        </motion.div>
      )}
      ...
      <div className="bg-white bg-opacity-10 rounded-lg p-4 border border-blue-500 border-opacity-30">
        ...
        <div className="space-y-2">
          <p className="text-white">
            <span className="text-blue-300 font-medium">{t("Topic")}:</span>{" "}
            {request}
          </p>
          <p className="text-white">
            <span className="text-blue-300 font-medium">{t("Language")}:</span>{" "}
            {t(selectedLanguage.label)}
          </p>
          <p className="text-white">
            <span className="text-blue-300 font-medium">{t("Level")}:</span>{" "}
            {t(selectedLevel.label)}
          </p>
          <p className="text-white">
            <span className="text-blue-300 font-medium">{t("Lessons")}:</span>{" "}
            {selectedLessonCount.label}
          </p>
          <p className="text-white">
            <span className="text-blue-300 font-medium">{t("Category")}:</span>{" "}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 flex flex-col items-center justify-start p-4">
      ...
      {showGuidance && (
        ...
        >
          <h2 className="text-2xl font-bold text-white mb-4">
            {t("How to Use the AI Course Generator")}
          </h2>
          <div className="space-y-4">
            <div className="bg-white bg-opacity-20 rounded-lg p-4">
              <h3 className="text-xl font-semibold text-white mb-2">
                {" "}
                {t("1.Describe Your Learning Goal")}
              </h3>
              <p className="text-blue-200">
                {t(
                  "Enter what you want to learn in the text area. Be specific about the subject and what aspects you want to focus on. For example: 'Learn Python for data science' or 'Understanding quantum physics basics'."
                )}
              </p>
            </div>
            ...
            <div className="bg-white bg-opacity-20 rounded-lg p-4">
              <h3 className="text-xl font-semibold text-white mb-2">
                {" "}
                {t("2.Check for Similar Courses")}
              </h3>
              <p className="text-blue-200">
                {t(
                  "As you type (3+ characters), the system automatically searches for similar existing courses. If you find what you're looking for, you can view that course instead of creating a new one."
                )}
              </p>
            </div>
            ...
            <div className="bg-white bg-opacity-20 rounded-lg p-4">
              <h3 className="text-xl font-semibold text-white mb-2">
                {" "}
                {t("3.Select Language and Level")}
              </h3>
              <p className="text-blue-200">
                {t(
                  "Choose your preferred language for the course content and select the appropriate difficulty level (Beginner, Intermediate, or Advanced)."
                )}
              </p>
            </div>
            ...
            <div className="bg-white bg-opacity-20 rounded-lg p-4">
              <h3 className="text-xl font-semibold text-white mb-2">
                {" "}
                {t("4.Create Your Course")}
              </h3>
              <p className="text-blue-200">
                {t(
                  "Click the 'Create your course' button to generate a personalized AI-driven course based on your inputs. Each course generation costs 1 credit."
                )}
              </p>
            </div>
            ...
            <div className="bg-white bg-opacity-20 rounded-lg p-4">
              <h3 className="text-xl font-semibold text-white mb-2">
                {" "}
                {t("5.Access Your Course")}
              </h3>
              <p className="text-blue-200">
                {t(
                  "Once generated, your course will appear in the 'Generated Courses' section. Click 'View Course' to start learning!"
                )}
              </p>
            </div>
            ...
            <div className="bg-purple-700 bg-opacity-40 rounded-lg p-4">
              ...
              <ul className="text-blue-200 list-disc pl-5 space-y-2">
                <li>
                  {t(
                    "Be specific in your request to get more targeted content"
                  )}
                </li>
                <li>
                  {t(
                    "Check your available credits before generating (shown at the top of the page)"
                  )}
                </li>
                <li>{t("Browse similar courses first to save credits")}</li>
                <li>
                  {t(
                    "If you need more credits, visit the Shop through the link in your profile"
                  )}
                </li>
              </ul>
            </div>
